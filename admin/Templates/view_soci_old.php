        ...
        <div class="card-body no-padding">
            ...
            <table class="datatable table table-striped primary" cellspacing="0" width="100%">
                ...
                <tr>
                    ...
                    <td class="text-center"><a href="soci.php?action=delete&id=<?= $socio->getId() ?>" onclick="javascript:return confirm('Vuoi veramente cancellare questo socio?')" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i> </a></td>
                </tr>
                <?php } } ?>
                <?php } else { ?>
                    <?php foreach ($array_non_soci as $chiave =>$socio) { ?>
                            <tr>
                                <td class="text-center">
                                    <input type="checkbox" class="form-control" name="iscritto_id" />
                                </td>
                                ...
                                <td class="text-center">
                                    <?php if($socio["somma"] > 0) { ?>
                                        <a href="uscite.php?action=list&id_socio=<?= $socio["id"] ?>" class="btn btn-xs btn-info"><i class="fa fa-archive"></i> <span class="badge"><?= $socio["somma"] ?></span></a>
                                    <?php } ?>
                                </td>
                                <td class="text-center"><a href="soci.php?action=edit&id=<?= $socio["id"] ?>" class="btn btn-xs btn-success"><i class="fa fa-pencil"></i> </a></td>
                                <td class="text-center"><a href="soci.php?action=delete&id=<?= $socio["id"] ?>" onclick="javascript:return confirm('Vuoi veramente cancellare questo perscatore non socio?')" class="btn btn-xs btn-danger"><i class="fa fa-trash"></i> </a></td>
                            </tr>
                        <?php } ?>
                <?php } ?>
                </tbody>
